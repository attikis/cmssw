...
//
//
// dummy producer for a L1TkEGParticle
// 
...
#include "L1Trigger/L1TTrackMatch/interface/L1TkEGEtComparator.h"
...
class L1TkEGParticleProducer : public edm::EDProducer {
public:
  ...
  explicit L1TkEGParticleProducer(const edm::ParameterSet&);
  ~L1TkEGParticleProducer();
...
//
L1TkEGParticleProducer::L1TkEGParticleProducer(const edm::ParameterSet& iConfig) :
  ...
  produces<L1TkEGParticleCollection>(label);
}

L1TkEGParticleProducer::~L1TkEGParticleProducer() {
}

// ------------ method called to produce the data  ------------
void
L1TkEGParticleProducer::produce(edm::Event& iEvent, const edm::EventSetup& iSetup) 
{
  using namespace edm;
  
  std::unique_ptr<L1TkEGParticleCollection> result(new L1TkEGParticleCollection);
  ...
  if (!L1TTTrackHandle.isValid() ) {
    LogError("L1TkEGParticleProducer")
  ...
  for ( unsigned int i=0; i < SelTTTrackPtrs.size(); i++ ){
	...
	// std::cout<<"Mass = "<<pionMass<<"   "<<p4tmp.M()<<std::endl;
      ...
      L1TkEGParticle trkEG(p4_total, TrackCluster, EGcluster, vtxIso);
...
// --------------------------------------------------------------------------------------
void L1TkEGParticleProducer::GetShrinkingConeSizes(float tk_pt,
						   const float shrinkCone_Constant,
						   const float sigCone_dRCutoff,
						   float &sigCone_dRMin,
						   float &sigCone_dRMax,
...
float L1TkEGParticleProducer::CalculateVtxIso(std::vector< L1TTTrackRefPtr > allTracks,
					      std::vector< unsigned int > clustTracksIndx,
					      bool useIsoCone) {
...
float L1TkEGParticleProducer::CorrectedEta(float eta, float zTrack)  {
...
void
L1TkEGParticleProducer::beginJob()
{
}

// ------------ method called once each job just after ending the event loop  ------------
void
L1TkEGParticleProducer::endJob() {
}

// ------------ method fills 'descriptions' with the allowed parameters for the module  ------------
void
L1TkEGParticleProducer::fillDescriptions(edm::ConfigurationDescriptions& descriptions) {
...
DEFINE_FWK_MODULE(L1TkEGParticleProducer);
