// -*- C++ -*-
//
// Package:    L1TkEGAnalyzer
// Class:      L1TkEGAnalyzer
// 
/**\class L1TkEGAnalyzer L1TkEGAnalyzer.cc SLHCUpgradeSimulations/L1TkEGAnalyzer/src/L1TkEGAnalyzer.cc
...
class L1TkEGAnalyzer : public edm::EDAnalyzer {
   public:

   typedef TTTrack< Ref_Phase2TrackerDigi_ >  L1TTTrackType;
   typedef std::vector< L1TTTrackType > L1TTTrackCollectionType;

  explicit L1TkEGAnalyzer(const edm::ParameterSet&);
  ~L1TkEGAnalyzer();
...
private:
  ...
  // template<class T1> 
  // std::vector<unsigned int> findMatchGenParticles(const T1 & tkObjCollection, const edm::Handle<reco::GenParticleCollection>& genH, 
  ...
  //edm::Handle<reco::GenParticleCollection> matchGenP;
  ...
  const edm::EDGetTokenT< L1TkEGParticleCollection > tkegToken;
  const edm::EDGetTokenT< reco::GenParticleCollection > genToken;

  int ievent; 
};

L1TkEGAnalyzer::L1TkEGAnalyzer(const edm::ParameterSet& iConfig) :
  trackToken(consumes< std::vector<TTTrack< Ref_Phase2TrackerDigi_> > > (iConfig.getParameter<edm::InputTag>("L1TrackInputTag"))),
  tkegToken(consumes< L1TkEGParticleCollection > (iConfig.getParameter<edm::InputTag>("L1TkEGInputTag"))),
...
void L1TkEGAnalyzer::beginJob() {
  ...
  if (analysisOption_ == "Efficiency") {
    ...
    // HistoName.str("");
    // HistoName << "PtTrack";
    // ptL1Track = fs->make<TH1F>(HistoName.str().c_str(), HistoName.str().c_str(), 60, 0.0, 300.0);
    // HistoName.str("");
    // HistoName << "PtTrackTurnOn";
    // ptL1TrackTurnOn = fs->make<TH1F>(HistoName.str().c_str(), HistoName.str().c_str(), 60, 0.0, 300.0);
...
void
L1TkEGAnalyzer::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup) {
  using namespace edm;
  ievent++;  
  ...
  */
  ...
  if (objectType_ == "TkEG"){
    ...
    edm::Handle< L1TkEGParticleCollection > l1TkEGHandle;
    iEvent.getByToken(tkegToken, l1TkEGHandle);
    L1TkEGParticleCollection l1TkEGCollection = (*l1TkEGHandle.product()); 
...
/*
void L1TkEGAnalyzer::checkTrackEfficiency(edm::Handle<L1TTTrackCollectionType> tkHandle) {
...
*/
template<class T1> 
void L1TkEGAnalyzer::checkEfficiency(const T1 & tkObjCollection) {
  ...
  // std::cout << " Selected Candidate : L1TkObject, L1TkObjectEtThr " << selectedL1TkObjTot_  << " " << selectedL1TkObjEtTot_ <<  std::endl;
  
  return;
}

template<class T1> 
void L1TkEGAnalyzer::checkRate(const T1 & tkObjCollection) {
  ...
  for (auto tkObjIter = tkObjCollection.begin(); tkObjIter != tkObjCollection.end(); ++tkObjIter) {  // not needed (could just use the first object - leading)
	...
	//if (et > 20.0) {
	etaL1TrkObj->Fill(eta); // not needed        
	phiL1TrkObj->Fill(phi); // not needed  
	//}      
      }
    }         
  }
...
/*
template<class T1, class T2> 
void L1TkEGAnalyzer::checkEfficiency(const T1 & objCollection, const T2 & tkObjCollection) {
  ...
  for (auto objIter = objCollection.begin(0);  objIter != objCollection.end(0); ++objIter) {
   ...
   // std::cout << " L1TkObject Info : dR Gen , et " << dRminTkObj << " " << etTkObj << std::endl;
   // std::cout << " Selected Candidates : L1Object, L1ObjectEt, L1TkObject, L1TkObjectEtThr " << selectedL1ObjTot_ << " " << selectedL1ObjEtTot_ <<  " " << selectedL1TkObjTot_  << " " << selectedL1TkObjEtTot_ <<  std::endl;
  
}


template<class T1, class T2> 
void L1TkEGAnalyzer::checkRate(const T1 & objCollection, const T2 & tkObjCollection) {
...
void L1TkEGAnalyzer::fillIntegralHistos(TH1F* th, float var){
  int nbin = th->FindBin(var); 
  for (int ibin = 1; ibin < nbin+1; ibin++) th->Fill(th->GetBinCenter(ibin));
}


std::vector<unsigned int> L1TkEGAnalyzer::findGenParticles(const edm::Handle<reco::GenParticleCollection>& genH, std::vector<float>& pt, std::vector<float>& eta, std::vector<float>& phi ) {
...
int L1TkEGAnalyzer::findGenParticle(const edm::Handle<reco::GenParticleCollection>& genH, float& pt, float & eta, float& phi) {
  int indx = -1;
  int pId = 0;
  if (objectType_ == "TkEG") 
    {
  ...
  for (size_t i = 0; i < (*genH.product()).size(); ++i) {
    ...
    if (abs(p.pdgId()) == pId )  {// && p.status() == 1) {
      indx=i;
      //std::cout << "Found the TAUUUU-----"<<std::endl;
...
//define thi sas a plug-in
DEFINE_FWK_MODULE(L1TkEGAnalyzer);
